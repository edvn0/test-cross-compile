...
public:
  ...
  [[nodiscard]]
  static auto create(std::filesystem::path const &library_path)
      -> std::expected<SlangLibrary, SlangLibraryError>;

  [[nodiscard]]
  static auto create_from_executable_directory(
      std::filesystem::path const &library_name = L"slang-compiler.dll")
      -> std::expected<SlangLibrary, SlangLibraryError>;

  [[nodiscard]]
  auto create_global_session(slang::IGlobalSession **session) const noexcept
      -> SlangResult;

  [[nodiscard]]
  auto create_global_session(SlangGlobalSessionDesc const &description,
                             slang::IGlobalSession **session) const noexcept
      -> SlangResult;
