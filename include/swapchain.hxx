...
private:
  ...
  [[nodiscard]]
  auto choose_surface_format(std::vector<VkSurfaceFormatKHR> const &formats)
      const noexcept -> VkSurfaceFormatKHR;

  [[nodiscard]]
  auto choose_present_mode(std::vector<VkPresentModeKHR> const &present_modes)
      const noexcept -> VkPresentModeKHR;

  [[nodiscard]]
  auto
  choose_extent(VkSurfaceCapabilitiesKHR const &capabilities) const noexcept
      -> VkExtent2D;
