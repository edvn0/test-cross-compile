...
auto make_error(PipelineErrorType type, VkResult result = VK_SUCCESS) noexcept
    -> PipelineError {
...
Pipeline::Pipeline(Pipeline &&other) noexcept
    : context_(std::exchange(other.context_, nullptr)),
      ...
      bind_point_(
          std::exchange(other.bind_point_, VK_PIPELINE_BIND_POINT_GRAPHICS)) {}
...
auto Pipeline::create_graphics(VulkanContext &context,
                               GraphicsPipelineCreateInfo const &create_info,
                               VkDescriptorSetLayout global_layout)
    -> std::expected<Pipeline, PipelineError> {
  if (context.device == VK_NULL_HANDLE || create_info.shaders.empty() ||
      create_info.colour_formats.empty()) {
    return std::unexpected(make_error(PipelineErrorType::invalid_argument));
  }

  std::vector<VkShaderModuleCreateInfo> shader_module_infos(
      create_info.shaders.size());
  ...
  layouts.insert(layouts.end(),
                 create_info.additional_descriptor_set_layouts.begin(),
  ...
  auto vk_result = vkCreatePipelineLayout(context.device, &layout_info, nullptr,
                                          &result.layout_);
  ...
  if (vk_result != VK_SUCCESS) {
    ...
    return std::unexpected(
        make_error(PipelineErrorType::layout_creation_failed, vk_result));
  }
  ...
  std::vector<VkPipelineColorBlendAttachmentState> blend_attachments(
      create_info.colour_formats.size());
  ...
  dynamic_states.insert_range(forward_dynamic_states);
  dynamic_states.insert_range(create_info.dynamic_states);
  ...
  VkPipelineRenderingCreateInfo const rendering_info{
      ...
      .colorAttachmentCount =
          static_cast<std::uint32_t>(create_info.colour_formats.size()),
  ...
  vk_result =
      vkCreateGraphicsPipelines(context.device, VK_NULL_HANDLE, 1,
                                &pipeline_info, nullptr, &result.pipeline_);
  ...
  if (vk_result != VK_SUCCESS) {
    ...
    return std::unexpected(
        make_error(PipelineErrorType::pipeline_creation_failed, vk_result));
  }

  result.bind_point_ = VK_PIPELINE_BIND_POINT_GRAPHICS;

  set_object_name(context.device, VK_OBJECT_TYPE_PIPELINE,
                  object_handle(result.pipeline_), create_info.debug_name);

  auto const layout_name = std::string{create_info.debug_name} + ".layout";

  set_object_name(context.device, VK_OBJECT_TYPE_PIPELINE_LAYOUT,
                  object_handle(result.layout_), layout_name);
