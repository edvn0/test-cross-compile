...
auto make_error(GpuResourceTableErrorType type,
                VkResult result = VK_SUCCESS) noexcept
    -> GpuResourceTableError {
  return {
      .type = type,
      .result = result,
  };
...
constexpr auto binding_index(GpuResourceBinding binding) noexcept
    -> std::uint32_t {
...
auto GpuResourceTable::create(VulkanContext &context,
                              GpuResourceTableCreateInfo const &create_info)
    -> std::expected<GpuResourceTable, GpuResourceTableError> {
  if (context.device == VK_NULL_HANDLE || create_info.frames_in_flight == 0 ||
      create_info.image_capacity == 0 || create_info.sampler_capacity == 0) {
    return std::unexpected(
        make_error(GpuResourceTableErrorType::invalid_argument));
  }
  ...
  auto result = vkCreateDescriptorSetLayout(context.device, &layout_info,
                                            nullptr, &table.layout_);

  if (result != VK_SUCCESS) {
    table.destroy();

    return std::unexpected(make_error(
        GpuResourceTableErrorType::descriptor_layout_creation_failed, result));
  }
  ...
  result =
      vkCreateDescriptorPool(context.device, &pool_info, nullptr, &table.pool_);

  if (result != VK_SUCCESS) {
    table.destroy();

    return std::unexpected(make_error(
        GpuResourceTableErrorType::descriptor_pool_creation_failed, result));
  }

  auto layouts = std::vector<VkDescriptorSetLayout>(frame_count, table.layout_);

  auto descriptor_sets =
      std::vector<VkDescriptorSet>(frame_count, VK_NULL_HANDLE);
  ...
  result = vkAllocateDescriptorSets(context.device, &allocate_info,
  ...
  if (result != VK_SUCCESS) {
    ...
    return std::unexpected(make_error(
        GpuResourceTableErrorType::descriptor_set_allocation_failed, result));
  }
...
auto GpuResourceTable::prepare_frame(std::uint32_t frame_index,
                                     ImageStorage const &images,
                                     SamplerStorage const &samplers)
    -> std::expected<void, GpuResourceTableError> {
  if (context_ == nullptr || frame_index >= frames_.size() ||
      images.capacity() > image_capacity_ ||
      samplers.capacity() > sampler_capacity_) {
    return std::unexpected(
        make_error(images.capacity() > image_capacity_ ||
                           samplers.capacity() > sampler_capacity_
  ...
  auto append_image_write = [&](std::uint32_t binding,
  ...
  for (std::uint32_t index = 0; index < images.capacity(); ++index) {
    ...
    auto const sampled_2d = record.sampled_2d != VK_NULL_HANDLE
                                ? record.sampled_2d
                                : fallback.sampled_2d;

    append_image_write(binding_index(GpuResourceBinding::sampled_2d), index,
                       VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, sampled_2d,
  ...
  auto const regular_fallback =
      samplers.descriptor_record(samplers.linear_repeat().index).sampler;

  auto const comparison_fallback =
      samplers.descriptor_record(samplers.shadow_compare().index).sampler;

  auto append_sampler_write =
      [&](std::uint32_t binding, std::uint32_t array_index, VkSampler sampler) {
        image_infos.push_back(VkDescriptorImageInfo{
            .sampler = sampler,
  ...
  for (std::uint32_t index = 0; index < samplers.capacity(); ++index) {
    ...
    auto const regular_sampler =
        record.occupied && record.sampler_class == SamplerClass::regular
            ? record.sampler
            : regular_fallback;

    auto const comparison_sampler =
        record.occupied && record.sampler_class == SamplerClass::comparison
            ? record.sampler
            : comparison_fallback;

    append_sampler_write(binding_index(GpuResourceBinding::samplers), index,
                         regular_sampler);

    append_sampler_write(binding_index(GpuResourceBinding::comparison_samplers),
                         index, comparison_sampler);
...
auto GpuResourceTable::bind(VkCommandBuffer command_buffer,
                            std::uint32_t frame_index,
                            VkPipelineBindPoint bind_point,
                            VkPipelineLayout pipeline_layout) const noexcept
    -> void {
  if (command_buffer == VK_NULL_HANDLE || pipeline_layout == VK_NULL_HANDLE ||
  ...
  vkCmdBindDescriptorSets(command_buffer, bind_point, pipeline_layout, 0, 1,
...
auto GpuResourceTable::operator=(GpuResourceTable &&other) noexcept
    -> GpuResourceTable & {
  if (this == &other) {
    return *this;
  }
