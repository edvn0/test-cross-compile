...
auto create_pipeline(
    ...
    return renderer.register_pipeline(
        PipelineRegisterInfo{
            .stages =
                {
...
public:
    explicit BulletDebugDraw(std::vector<Vertex> &pending_lines) noexcept
        : pending_lines_{pending_lines} {}

    void drawLine(
        btVector3 const &from,
        btVector3 const &to,
        btVector3 const &colour
    ) override {
    ...
    void drawContactPoint(
        btVector3 const &point_on_b,
        btVector3 const &normal_on_b,
        btScalar distance,
        int /*life_time*/,
        btVector3 const &colour
    ) override {
        drawLine(
            point_on_b,
            point_on_b + normal_on_b * distance,
            colour
        );
        ...
        drawLine(
            point_on_b - btVector3{marker_radius, 0, 0},
            point_on_b + btVector3{marker_radius, 0, 0},
            colour
        );

        drawLine(
            point_on_b - btVector3{0, marker_radius, 0},
            point_on_b + btVector3{0, marker_radius, 0},
            colour
        );

        drawLine(
            point_on_b - btVector3{0, 0, marker_radius},
            point_on_b + btVector3{0, 0, marker_radius},
            colour
        );
    }

    void reportErrorWarning(char const *warning_string) override {
        warn("(Bullet) {}", warning_string);
    }
...
struct DebugRenderer::Impl {
    ...
    explicit Impl(Renderer &r)
        : renderer{r},
          bullet_debug_draw{pending_lines} {
        frame_buffers.resize(
            renderer.context().swapchain.frame_count()
        );
    }
    ...
    auto ensure_capacity(
        ...
        if (frame_index >= frame_buffers.size()) {
            error(
                "[DebugDraw] Invalid frame index {} for {} frame buffers",
                frame_index,
                frame_buffers.size()
            );
            return false;
        ...
        if (
            frame_buffer.vertex != nullptr &&
            frame_buffer.capacity >= vertex_count
        ) {
            return true;
        }

        auto const size = next_power_of_two(
            static_cast<std::size_t>(vertex_count) * sizeof(Vertex)
        );

        info(
            "[DebugDraw] Reallocating line buffer to {} bytes",
            size
        );

        auto created = Buffer::create(
            renderer.context(),
            BufferCreateInfo{
                .size = size,
                .usage =
        ...
        frame_buffer.vertex =
            std::make_unique<Buffer>(std::move(*created));

        frame_buffer.capacity =
            static_cast<std::uint32_t>(size / sizeof(Vertex));
...
auto DebugRenderer::operator=(DebugRenderer &&) noexcept
    -> DebugRenderer & = default;

auto DebugRenderer::bullet_debug_draw() noexcept -> btIDebugDraw * {
    return &impl_->bullet_debug_draw;
}

auto DebugRenderer::begin_frame() -> void {
    impl_->pending_lines.clear();
}

auto DebugRenderer::render(
    VkCommandBuffer cmd,
...
auto DebugRenderer::render(
    ...
    if (
        impl_->force_recompile ||
        !impl_->pipeline.valid()
    ) {
        auto created = create_pipeline(
            impl_->renderer,
            impl_->renderer.hdr_format(),
            impl_->renderer.depth_format()
        );
    ...
    auto const vertex_count =
        static_cast<std::uint32_t>(impl_->pending_lines.size());
    ...
    if (!frame_buffer.vertex->write(
            0,
            std::span<const Vertex>{impl_->pending_lines}
        )) {
    ...
    auto const *pipeline =
        impl_->renderer.resolve_pipeline(impl_->pipeline);
    ...
    auto const &extent =
        impl_->renderer.context().swapchain.extent();
    ...
    impl_->renderer.resource_table().bind(
        cmd,
        frame_index,
        VK_PIPELINE_BIND_POINT_GRAPHICS,
        pipeline->layout()
...
auto DebugRenderer::clear_lines() -> void {
    impl_->bullet_debug_draw.clearLines();
}
