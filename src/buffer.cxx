...
[[nodiscard]]
auto make_allocation_create_info(BufferMemory memory)
...
[[nodiscard]]
auto set_buffer_debug_name(VulkanContext const &ctx, VkBuffer buffer,
                           char const *debug_name) -> VkResult {
...
[[nodiscard]]
auto make_buffer_error(std::string_view message, VkResult result)
    -> DeviceError {
...
Buffer::Buffer(Buffer &&other) noexcept
    : buffer(std::exchange(other.buffer, VK_NULL_HANDLE)),
      ...
      allocation_info(
          std::exchange(other.allocation_info, VmaAllocationInfo{})),
...
auto Buffer::validate_range(VkDeviceSize offset,
                            VkDeviceSize range_size) const noexcept -> bool {
  if (!valid() || !mapped()) {
    return false;
  }
...
auto Buffer::write(VkDeviceSize offset, std::span<const std::byte> data)
  ...
  if (!validate_range(offset, write_size)) {
    return std::unexpected{make_buffer_error("Buffer write range is invalid",
...
auto Buffer::read(VkDeviceSize offset, std::span<std::byte> destination)
    -> std::expected<void, DeviceError> {
  ...
  if (!validate_range(offset, read_size)) {
    return std::unexpected{make_buffer_error("Buffer read range is invalid",
                                             VK_ERROR_MEMORY_MAP_FAILED)};
  }
  ...
  std::memcpy(destination.data(), mapped_data() + offset,
              destination.size_bytes());

  return {};
}

auto Buffer::flush(VkDeviceSize offset, VkDeviceSize flush_size)
    -> std::expected<void, DeviceError> {
  if (!validate_range(offset, flush_size)) {
    return std::unexpected{make_buffer_error("Buffer flush range is invalid",
                                             VK_ERROR_MEMORY_MAP_FAILED)};
  }

  auto const result =
      vmaFlushAllocation(allocator, allocation, offset, flush_size);

  if (result != VK_SUCCESS) {
    return std::unexpected{
        make_buffer_error("vmaFlushAllocation failed", result)};
  }

  return {};
}

auto Buffer::invalidate(VkDeviceSize offset, VkDeviceSize invalidate_size)
    -> std::expected<void, DeviceError> {
  if (!validate_range(offset, invalidate_size)) {
    return std::unexpected{make_buffer_error(
        "Buffer invalidate range is invalid", VK_ERROR_MEMORY_MAP_FAILED)};
  }

  auto const result =
      vmaInvalidateAllocation(allocator, allocation, offset, invalidate_size);

  if (result != VK_SUCCESS) {
    return std::unexpected{
        make_buffer_error("vmaInvalidateAllocation failed", result)};
  }
...
auto Buffer::create(VulkanContext &ctx, BufferCreateInfo const &create_info)
    -> std::expected<Buffer, DeviceError> {
  if (create_info.size == 0) {
    return std::unexpected{
        make_buffer_error("Buffer size must be greater than zero",
                          VK_ERROR_INITIALIZATION_FAILED)};
  }

  if (create_info.usage == 0) {
    return std::unexpected{make_buffer_error("Buffer usage must not be zero",
                                             VK_ERROR_INITIALIZATION_FAILED)};
  }
  ...
  auto const vk_result = vmaCreateBuffer(
      ctx.allocator, &vk_create_info, &allocation_create_info, &result.buffer,
  ...
  if (vk_result != VK_SUCCESS) {
    ...
    return std::unexpected{
        make_buffer_error("vmaCreateBuffer failed", vk_result)};
  }
  ...
  if (result.allocation_info.pMappedData == nullptr) {
    ...
    return std::unexpected{
        make_buffer_error("VMA returned an unmapped buffer allocation",
                          VK_ERROR_MEMORY_MAP_FAILED)};
  }
  ...
  if (!create_info.debug_name.empty()) {
    ...
    static_cast<void>(
        set_buffer_debug_name(ctx, result.buffer, debug_name.c_str()));
  }
  ...
  if (create_info.usage & VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT) {
    ...
    if (result.device_address == 0) {
      ...
      return std::unexpected{
          make_buffer_error("vkGetBufferDeviceAddress returned zero",
                            VK_ERROR_INITIALIZATION_FAILED)};
    }
  }
