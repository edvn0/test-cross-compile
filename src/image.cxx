...
namespace {
[[nodiscard]]
auto descriptor_view_type(ImageDescriptorView type) noexcept
    -> VkImageViewType {
  switch (type) {
...
[[nodiscard]]
auto descriptor_view_usage_is_valid(ImageCreateInfo const &create_info,
                                    ImageDescriptorView type) noexcept -> bool {
  switch (type) {
  case ImageDescriptorView::sampled_2d:
...
auto make_error(ImageErrorType type, VkResult result = VK_SUCCESS) noexcept
    -> ImageError {
  return ImageError{
      .type = type,
      .result = result,
...
auto set_object_name(VkDevice device, VkObjectType object_type,
...
Image::Image(Image &&other) noexcept
    : context_(std::exchange(other.context_, nullptr)),
      ...
      allocation_info_(
          std::exchange(other.allocation_info_, VmaAllocationInfo{})),
...
auto Image::create(VulkanContext &context, ImageCreateInfo const &create_info)
  ...
  if (context.device == VK_NULL_HANDLE || context.allocator == VK_NULL_HANDLE ||
      ...
      create_info.format == VK_FORMAT_UNDEFINED || create_info.usage == 0 ||
      create_info.mip_levels == 0 || create_info.array_layers == 0) {
    return std::unexpected(make_error(ImageErrorType::invalid_argument));
  }

  auto const aspect = create_info.aspect != 0
                          ? create_info.aspect
                          : infer_aspect(create_info.format);
  ...
  auto result = vmaCreateImage(context.allocator, &image_info, &allocation_info,
                               &image.image_, &image.allocation_,
  ...
  if (result != VK_SUCCESS) {
    ...
    return std::unexpected(
        make_error(ImageErrorType::image_creation_failed, result));
  }
  ...
  if (result != VK_SUCCESS) {
    ...
    return std::unexpected(
        make_error(ImageErrorType::view_creation_failed, result));
  }

  for (std::uint32_t raw_type = 0;
       raw_type < static_cast<std::uint32_t>(ImageDescriptorView::count);
       ++raw_type) {
    ...
    auto const descriptor_view_name = std::string{create_info.debug_name} +
                                      ".descriptor_view." +
                                      std::to_string(raw_type);

    set_object_name(
        context.device, VK_OBJECT_TYPE_IMAGE_VIEW,
        object_handle(image.descriptor_views_[descriptor_view_index]),
        descriptor_view_name);
  }
  ...
  set_object_name(context.device, VK_OBJECT_TYPE_IMAGE,
                  object_handle(image.image_), create_info.debug_name);

  auto const view_name = std::string{create_info.debug_name} + ".view";

  set_object_name(context.device, VK_OBJECT_TYPE_IMAGE_VIEW,
                  object_handle(image.view_), view_name);
...
auto Image::destroy() noexcept -> void {
  ...
  for (auto &descriptor_view : descriptor_views_) {
    if (descriptor_view != VK_NULL_HANDLE &&
