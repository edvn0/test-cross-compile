...
[[nodiscard]]
constexpr auto index_element_size(VkIndexType index_type) noexcept
    -> VkDeviceSize {
  switch (index_type) {
...
auto GeometryArena::create(VulkanContext &ctx,
...
auto GeometryArena::allocate_bytes(VkDeviceSize allocation_size,
                                   VkDeviceSize alignment)
    -> std::expected<GeometrySlice, GeometryArenaError> {
  if (allocation_size == 0 || alignment == 0 ||
  ...
  if (next_offset >
      std::numeric_limits<VkDeviceSize>::max() - (alignment - 1)) {
    return std::unexpected{GeometryArenaError{
...
auto GeometryArena::write(GeometrySlice const &slice,
                          std::span<const std::byte> data)
    -> std::expected<void, GeometryArenaError> {
  if (data.size_bytes() != slice.size || slice.offset > capacity ||
      slice.size > capacity - slice.offset) {
    return std::unexpected{GeometryArenaError{
        .type = GeometryArenaErrorType::invalid_argument,
    }};
  }
...
auto GeometryArena::allocate_vertices(std::span<const std::byte> data,
                                      std::uint32_t vertex_stride,
                                      VkDeviceSize alignment)
    -> std::expected<VertexSlice, GeometryArenaError> {
  if (data.empty() || vertex_stride == 0 ||
  ...
  auto allocation =
      allocate_bytes(static_cast<VkDeviceSize>(data.size_bytes()), alignment);
...
auto GeometryArena::allocate_indices(std::span<const std::byte> data,
                                     VkIndexType index_type)
    -> std::expected<IndexSlice, GeometryArenaError> {
  auto const element_size = index_element_size(index_type);

  if (element_size == 0 || data.empty() ||
      data.size_bytes() % element_size != 0) {
    return std::unexpected{GeometryArenaError{
        .type = element_size == 0
                    ? GeometryArenaErrorType::unsupported_index_type
  ...
  auto allocation = allocate_bytes(static_cast<VkDeviceSize>(data.size_bytes()),
                                   element_size);
