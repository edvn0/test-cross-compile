...
auto make_pipeline_error(PipelineError const &error) noexcept
    -> PipelineStorageError {
  return PipelineStorageError{
...
auto PipelineStorage::create(VulkanContext &context,
                             PipelineStorageCreateInfo const &create_info)
  ...
  if (context.device == VK_NULL_HANDLE || create_info.capacity == 0 ||
    ...
    return std::unexpected(
        make_error(PipelineStorageErrorType::invalid_argument));
  }
  ...
  storage.global_descriptor_set_layout_ =
  ...
  for (std::uint32_t index = 0; index < create_info.capacity; ++index) {
    storage.slots_[index].next_free =
        index + 1 < create_info.capacity ? index + 1 : create_info.capacity;
  }
...
auto PipelineStorage::create_graphics(
...
auto PipelineStorage::destroy_pipeline(PipelineHandle handle)
    -> std::expected<void, PipelineStorageError> {
  auto *slot = slot_for(handle);

  if (slot == nullptr) {
    return std::unexpected(
        make_error(PipelineStorageErrorType::invalid_handle));
  }
...
auto PipelineStorage::get(PipelineHandle handle) const noexcept
    -> Pipeline const * {
...
auto PipelineStorage::slot_for(PipelineHandle handle) const noexcept
    -> Slot const * {
  if (!handle.valid() || handle.index >= slots_.size()) {
    return nullptr;
  }
