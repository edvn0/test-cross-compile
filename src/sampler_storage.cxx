...
auto make_error(SamplerStorageErrorType type,
                VkResult result = VK_SUCCESS) noexcept -> SamplerStorageError {
...
auto default_sampler_infos()
    -> std::array<DefaultSamplerInfo, default_sampler_count> {
  return {
      DefaultSamplerInfo{
          .create_info =
              SamplerCreateInfo{
...
auto create_vk_sampler(VulkanContext &context,
                       SamplerCreateInfo const &create_info)
  ...
  if (context.device == VK_NULL_HANDLE || create_info.max_anisotropy < 1.0F ||
    ...
    return std::unexpected(
        make_error(SamplerStorageErrorType::invalid_argument));
  }
  ...
  VkSamplerCreateInfo const sampler_info{
      ...
      .anisotropyEnable =
          create_info.max_anisotropy > 1.0F ? VK_TRUE : VK_FALSE,
      ...
      .borderColor = comparison ? VK_BORDER_COLOR_FLOAT_OPAQUE_BLACK
  ...
  if (result != VK_SUCCESS) {
    return std::unexpected(
        make_error(SamplerStorageErrorType::sampler_creation_failed, result));
  }

  set_object_name(context.device, VK_OBJECT_TYPE_SAMPLER,
                  object_handle(sampler), create_info.debug_name);
...
auto SamplerStorage::operator=(SamplerStorage &&other) noexcept
    -> SamplerStorage & {
  if (this == &other) {
    return *this;
  }
...
auto SamplerStorage::create(VulkanContext &context, std::uint32_t capacity,
  ...
  for (std::uint32_t index = 0; index < default_sampler_count; ++index) {
    ...
    auto const full_name =
        storage.debug_name_ + ".default." + std::string{create_info.debug_name};
  ...
  if (capacity > default_sampler_count) {
    ...
    for (std::uint32_t index = default_sampler_count; index < capacity;
         ++index) {
...
auto SamplerStorage::create_sampler(SamplerCreateInfo const &create_info)
  ...
  if (free_head_ == 0) {
    return std::unexpected(
        make_error(SamplerStorageErrorType::capacity_exceeded));
  }
...
auto SamplerStorage::descriptor_record(std::uint32_t index) const noexcept
...
auto SamplerStorage::destroy_sampler(SamplerHandle handle)
  ...
  if (slot.protected_default) {
    return std::unexpected(
        make_error(SamplerStorageErrorType::protected_default));
  }
