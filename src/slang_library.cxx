...
using SlangCreateGlobalSessionFunction = SlangResult (*)(
    SlangGlobalSessionDesc const *description, slang::IGlobalSession **session);

[[nodiscard]]
auto make_error(SlangLibraryErrorType type, SlangResult result,
                std::string diagnostics) -> SlangLibraryError {
...
auto wide_to_utf8(std::wstring_view text) -> std::string {
  ...
  auto const required_size = WideCharToMultiByte(
  ...
  auto const written_size = WideCharToMultiByte(
      CP_UTF8, WC_ERR_INVALID_CHARS, text.data(), static_cast<int>(text.size()),
...
auto windows_error_message(DWORD error) -> std::string {
  ...
  auto const character_count = FormatMessageW(
...
auto executable_path()
    -> std::expected<std::filesystem::path, SlangLibraryError> {
...
[[nodiscard]]
auto absolute_path(std::filesystem::path const &path)
    -> std::expected<std::filesystem::path, SlangLibraryError> {
  ...
  if (error_code) {
    return std::unexpected{
        make_error(SlangLibraryErrorType::invalid_argument, SLANG_E_INVALID_ARG,
...
SlangLibrary::SlangLibrary(std::unique_ptr<Impl> impl) noexcept
    : impl_{std::move(impl)} {}

SlangLibrary::~SlangLibrary() { destroy(); }

SlangLibrary::SlangLibrary(SlangLibrary &&other) noexcept
...
auto SlangLibrary::create(std::filesystem::path const &library_path)
    -> std::expected<SlangLibrary, SlangLibraryError> {
  if (library_path.empty()) {
    return std::unexpected{make_error(SlangLibraryErrorType::invalid_argument,
                                      SLANG_E_INVALID_ARG,
  ...
  debug("Path starts with UNC prefix: {}", native_path.size() >= 2 &&
                                               native_path[0] == L'\\' &&
                                               native_path[1] == L'\\');
    ...
    return std::unexpected{
        make_error(SlangLibraryErrorType::library_not_found, SLANG_E_NOT_FOUND,
                   std::format("The Slang runtime library was not found: {}\n"
                               ...
                               "(Windows error {})",
                               wide_to_utf8(native_path),
                               windows_error_message(error), error))};
  }

  if ((attributes & FILE_ATTRIBUTE_DIRECTORY) != 0) {
    return std::unexpected{make_error(
        SlangLibraryErrorType::library_not_found, SLANG_E_NOT_FOUND,
        std::format("The Slang runtime path refers to a directory: {}",
                    wide_to_utf8(native_path)))};
  }
  ...
  if (impl->module == nullptr) {
    ...
    return std::unexpected{make_error(
        SlangLibraryErrorType::library_load_failed, SLANG_E_NOT_AVAILABLE,
        std::format("Failed to load the Slang runtime library '{}': "
                    "{} (Windows error {})",
                    wide_to_utf8(native_path), windows_error_message(error),
                    error))};
  }
  ...
  auto const symbol =
      GetProcAddress(impl->module, "slang_createGlobalSession2");
  ...
  if (symbol == nullptr) {
    ...
    return std::unexpected{
        make_error(SlangLibraryErrorType::symbol_not_found, SLANG_E_NOT_FOUND,
  ...
  impl->create_global_session =
      std::bit_cast<SlangCreateGlobalSessionFunction>(symbol);
...
auto SlangLibrary::create_from_executable_directory(
    std::filesystem::path const &library_name)
    -> std::expected<SlangLibrary, SlangLibraryError> {
  ...
  if (separator == std::wstring::npos) {
    return std::unexpected{make_error(
        ...
        std::format("The executable path has no directory separator: {}",
                    wide_to_utf8(executable_native)))};
  }
  ...
  debug("Slang path starts with UNC prefix: {}",
        executable_native.size() >= 2 && executable_native[0] == L'\\' &&
            executable_native[1] == L'\\');

  return create(std::filesystem::path{std::move(executable_native)});
}

auto SlangLibrary::create_global_session(
...
auto SlangLibrary::create_global_session(
    SlangGlobalSessionDesc const &description,
    slang::IGlobalSession **session) const noexcept -> SlangResult {
  if (session == nullptr) {
