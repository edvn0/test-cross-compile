...
auto checked_buffer_size(std::uint32_t capacity)
    -> std::expected<VkDeviceSize, MaterialStorageError> {
  ...
  if (capacity == 0 || static_cast<VkDeviceSize>(capacity) > maximum / stride) {
    return std::unexpected(
        make_error(MaterialStorageErrorType::invalid_argument));
  }
...
auto MaterialStorage::operator=(MaterialStorage &&other) noexcept
    -> MaterialStorage & {
  if (this == &other) {
    return *this;
  }
...
auto MaterialStorage::create(VulkanContext &context,
  ...
  std::string const upload_name =
      std::string{create_info.debug_name} + ".upload";
...
auto MaterialStorage::create_material(GpuMaterial const &material)
  ...
  if (free_head_ == 0) {
    return std::unexpected(
        make_error(MaterialStorageErrorType::capacity_exceeded));
  }
...
auto MaterialStorage::update_material(MaterialHandle handle,
                                      GpuMaterial const &material)
    -> std::expected<void, MaterialStorageError> {
  auto *slot = slot_for(handle);

  if (slot == nullptr) {
    return std::unexpected(
        make_error(MaterialStorageErrorType::invalid_handle));
  }
...
auto MaterialStorage::destroy_material(MaterialHandle handle)
    -> std::expected<void, MaterialStorageError> {
...
auto MaterialStorage::get(MaterialHandle handle) const noexcept
    -> GpuMaterial const * {
...
auto MaterialStorage::gpu_index(MaterialHandle handle) const noexcept
    -> std::uint32_t {
  return slot_for(handle) != nullptr ? handle.index : 0;
}

auto MaterialStorage::prepare_frame(VkCommandBuffer command_buffer,
                                    std::uint32_t frame_index)
    -> std::expected<void, MaterialStorageError> {
  static_cast<void>(frame_index);

  if (command_buffer == VK_NULL_HANDLE ||
      upload_buffer_.allocation_info.pMappedData == nullptr) {
    return std::unexpected(
        make_error(MaterialStorageErrorType::invalid_argument));
  }
  ...
  auto *mapped =
      static_cast<std::byte *>(upload_buffer_.allocation_info.pMappedData);
  ...
  for (auto const &range : dirty_ranges) {
    auto const offset =
        static_cast<VkDeviceSize>(range.first) * sizeof(GpuMaterial);

    auto const size =
        static_cast<VkDeviceSize>(range.count) * sizeof(GpuMaterial);
...
auto MaterialStorage::slot_for(MaterialHandle handle) const noexcept
    -> Slot const * {
  if (handle.index >= slots_.size()) {
    return nullptr;
  }
