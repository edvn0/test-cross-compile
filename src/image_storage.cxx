...
namespace {
constexpr std::array<std::array<std::uint8_t, 4>, default_image_count>
    default_pixels{{
        // 0: white
        {255, 255, 255, 255},
...
constexpr std::array<std::string_view, default_image_count> default_names{
    "white",     "black",    "flat_normal", "metallic_roughness",
    "occlusion", "emissive",
};
...
auto ImageStorage::create(VulkanContext &context,
                          ImageStorageCreateInfo const &create_info)
  ...
  if (create_info.capacity > default_image_count) {
    ...
    for (std::uint32_t index = default_image_count;
         index < create_info.capacity; ++index) {
      storage.slots_[index].next_free =
          index + 1 < create_info.capacity ? index + 1 : 0;
    }
  }
...
auto ImageStorage::create_default_images()
    -> std::expected<void, ImageStorageError> {
  constexpr auto upload_size = static_cast<VkDeviceSize>(
      default_pixels.size() * sizeof(default_pixels[0]));

  auto upload = Buffer::create(*context_,
                               BufferCreateInfo{
                                   .size = upload_size,
  ...
  std::memcpy(mapped, default_pixels.data(),
              static_cast<std::size_t>(upload_size));

  auto const flush_result = vmaFlushAllocation(
      context_->allocator, upload->allocation, 0, upload_size);
  ...
  for (std::uint32_t index = 0; index < default_image_count; ++index) {
    auto const name =
        debug_name_ + ".default." + std::string{default_names[index]};
    ...
    auto image = Image::create(
        *context_, ImageCreateInfo{
                       ...
                       .descriptor_views = image_descriptor_view_bit(
                           ImageDescriptorView::sampled_2d),
                       .flags = 0,
...
auto ImageStorage::create_image(ImageCreateInfo const &create_info)
  ...
  if (free_head_ == 0) {
    return std::unexpected(
        make_error(ImageStorageErrorType::capacity_exceeded));
  }
...
auto ImageStorage::destroy_image(ImageHandle handle)
    -> std::expected<void, ImageStorageError> {
  ...
  if (slot->protected_default) {
    return std::unexpected(
        make_error(ImageStorageErrorType::protected_default));
  }
...
auto ImageStorage::prepare_frame(VkCommandBuffer command_buffer)
    -> std::expected<void, ImageStorageError> {
  ...
  if (context_ == nullptr || command_buffer == VK_NULL_HANDLE ||
      default_upload_buffer_.buffer == VK_NULL_HANDLE) {
    return std::unexpected(make_error(ImageStorageErrorType::invalid_argument));
  }
  ...
  for (std::uint32_t index = 0; index < default_image_count; ++index) {
    VkBufferImageCopy2 const region{
        ...
        .pNext = nullptr,
        .bufferOffset =
            static_cast<VkDeviceSize>(index * sizeof(default_pixels[0])),
        .bufferRowLength = 0,
        .bufferImageHeight = 0,
        .imageSubresource =
...
auto ImageStorage::descriptor_record(std::uint32_t index) const noexcept
    -> ImageDescriptorRecord {
  ...
  return ImageDescriptorRecord{
      ...
      .sampled_cube =
          slot.image.descriptor_view(ImageDescriptorView::sampled_cube),
      .sampled_2d_array =
          slot.image.descriptor_view(ImageDescriptorView::sampled_2d_array),
      .storage_2d = slot.image.descriptor_view(ImageDescriptorView::storage_2d),
      .storage_2d_array =
          slot.image.descriptor_view(ImageDescriptorView::storage_2d_array),
      .revision = slot.revision,
      .occupied = true,
  };
}

auto ImageStorage::descriptor_revision(std::uint32_t index) const noexcept
    -> std::uint64_t {
  if (index >= slots_.size()) {
    return 0;
  }
