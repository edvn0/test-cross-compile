...
auto format_model_load_profile(ModelLoadProfile const &profile) -> std::string {
    auto const cpu_parse_total_ms = to_ms(profile.gltf_parse_ns) + to_ms(profile.material_resolve_ns) +
                                    ...
                                    to_ms(profile.lod_generation_ns) + to_ms(profile.vertex_compression_ns) +
                                    to_ms(profile.meshlet_build_ns);
    ...
    return std::format(
            ...
            "/ tangents {:.2f} / lods {:.2f} / vertex compress {:.2f} / meshlets {:.2f})\n"
            "  gpu upload:       {:.2f} ms across {} frame(s)  (material create {:.2f} / geometry upload {:.2f})\n"
            ...
            to_ms(profile.tangent_generation_ns), to_ms(profile.lod_generation_ns), to_ms(profile.vertex_compression_ns),
            to_ms(profile.meshlet_build_ns), gpu_upload_total_ms,
            profile.gpu_upload_frames.load(std::memory_order_relaxed), to_ms(profile.material_creation_ns),
            to_ms(profile.geometry_upload_ns), texture_cpu_total_ms,
