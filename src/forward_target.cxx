...
auto make_image_error(ImageStorageError const &error) noexcept
    -> ForwardTargetError {
  return ForwardTargetError{
...
auto ForwardTarget::operator=(ForwardTarget &&other) noexcept
    -> ForwardTarget & {
  if (this == &other) {
    return *this;
  }
...
auto ForwardTarget::create(ImageStorage &image_storage,
                           ForwardTargetCreateInfo const &create_info)
    -> std::expected<ForwardTarget, ForwardTargetError> {
  if (create_info.extent.width == 0 || create_info.extent.height == 0 ||
      create_info.hdr_format == VK_FORMAT_UNDEFINED ||
      create_info.depth_format == VK_FORMAT_UNDEFINED) {
    return std::unexpected(
        make_error(ForwardTargetErrorType::invalid_argument));
  }
