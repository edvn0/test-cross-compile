...
auto Swapchain::initialize(const SwapchainCreateInfo &create_info) noexcept
    -> bool {
  destroy();
  ...
  if (create_info.physical_device == VK_NULL_HANDLE ||
      ...
      create_info.graphics_queue == VK_NULL_HANDLE ||
  ...
  if (!create_swapchain(VK_NULL_HANDLE) || !create_image_views() ||
      !create_command_resources() || !create_synchronization()) {
    destroy();
    return false;
  }

  info("Swapchain created: {}x{}, {} images, format {}", extent_.width,
       extent_.height, images_.size(),
       static_cast<int>(surface_format_.format));

  return true;
}

auto Swapchain::begin_frame() noexcept
    -> std::expected<SwapchainFrame, SwapchainBeginFrameError> {
  ...
  auto result = vkWaitForFences(device_, 1, &frame.in_flight, VK_TRUE,
...
auto Swapchain::end_frame(SwapchainFrame const &active_frame) noexcept
    -> SwapchainFrameResult {
  if (active_frame.frame_index >= frames_.size() ||
      active_frame.image_index >= render_finished_semaphores_.size() ||
      active_frame.command_buffer == VK_NULL_HANDLE ||
  ...
  auto const render_finished =
      render_finished_semaphores_[active_frame.image_index];
  ...
  auto const advance_frame = [this] {
    current_frame_ =
        (current_frame_ + 1) % static_cast<std::uint32_t>(frames_.size());
  };
  ...
  if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR ||
      active_frame.acquire_suboptimal) {
...
auto Swapchain::create_swapchain(VkSwapchainKHR old_swapchain) noexcept
    -> bool {
  ...
  std::uint32_t image_count =
      std::max(requested, capabilities.minImageCount + 1);
  ...
  const VkSwapchainCreateInfoKHR create_info{
      ...
      .imageSharingMode = separate_queue_families ? VK_SHARING_MODE_CONCURRENT
                                                  : VK_SHARING_MODE_EXCLUSIVE,
      .queueFamilyIndexCount =
          separate_queue_families
  ...
  result = vkGetSwapchainImagesKHR(device_, swapchain_, &actual_image_count,
                                   nullptr);
...
auto Swapchain::create_image_views() noexcept -> bool {
  ...
  for (std::size_t index = 0; index < images_.size(); ++index) {
    ...
    const VkResult result =
        vkCreateImageView(device_, &create_info, nullptr, &image_views_[index]);
...
auto Swapchain::recreate() noexcept -> bool {
  ...
  info("Swapchain recreated: {}x{}, {} images", extent_.width, extent_.height,
       images_.size());
...
auto Swapchain::choose_surface_format(
  ...
  for (VkFormat preferred_format : preferred_formats) {
    const auto iterator = std::ranges::find_if(
        formats, [preferred_format](const VkSurfaceFormatKHR &format) {
...
auto Swapchain::choose_present_mode(
    const std::vector<VkPresentModeKHR> &present_modes) const noexcept
    -> VkPresentModeKHR {
  if (!vsync_) {
    const auto mailbox =
...
auto Swapchain::choose_extent(
    const VkSurfaceCapabilitiesKHR &capabilities) const noexcept -> VkExtent2D {
  ...
  return VkExtent2D{
      .width =
          std::clamp(requested_extent_.width, capabilities.minImageExtent.width,
                     capabilities.maxImageExtent.width),
      .height = std::clamp(requested_extent_.height,
